...
# === Setup Output Directory using centralized report_path module ===
base_dir = os.path.dirname(os.path.abspath(__file__))

# Use centralized report path - creates subdirectory for IBERT test
...
print_header("Eye Scan File Processing")
import shutil
...
# Validate eye scan files exist
print_info("  Checking eye scan CSV files...")
scan00_exists = validate_eye_scan_file(scan00_src, "X0Y4", result_dict)
scan01_exists = validate_eye_scan_file(scan01_src, "X0Y5", result_dict)
...
try:
    if scan00_exists:
        ...
        print_pass(f"  ✓ Copied: scan00.csv -> scan00_X0Y4.csv")
except Exception as e:
    print_fail(f"  ✗ Could not copy scan00.csv: {e}")

try:
    if scan01_exists:
        shutil.copy2(scan01_src, scan01_dst)
        print_pass(f"  ✓ Copied: scan01.csv -> scan01_X0Y5.csv")
except Exception as e:
    print_fail(f"  ✗ Could not copy scan01.csv: {e}")
